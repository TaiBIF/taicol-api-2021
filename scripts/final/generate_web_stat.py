...
# NOTE
# 「收錄物種數」可以詳盡一點的，所以希望能計算到種下(排除有種下的種階層)
# 其他的統計則是比較為主，因此計算到種階層就好。
...
with conn.cursor() as cursor:
    query = f"""SELECT COUNT(*) FROM `references` WHERE `type` != 4 AND deleted_at IS NULL"""
    ...
    query = """SELECT COUNT(at.taxon_id) FROM api_taxon at
                ...
                WHERE r.id >= 34 AND at.is_in_taiwan = 1 AND at.is_deleted != 1 AND at.taxon_id NOT IN (
    ...
    # 收錄學名數
    query = "SELECT COUNT(id) FROM taxon_names WHERE deleted_at IS NULL"
    cursor.execute(query)
...
with conn.cursor() as cursor:
    query = f"""SELECT r.key, COUNT(DISTINCT(at.taxon_id)) FROM api_taxon at
                JOIN ranks r ON at.rank_id = r.id
                WHERE at.rank_id IN (3,12,18,22,26,30,34) AND at.is_in_taiwan = 1 AND at.is_deleted != 1 
                GROUP BY at.rank_id
    ...
    for r in rank_count:
        query = f"INSERT INTO api_web_stat (title, category, count) VALUES ('rank_count', '{r[0].capitalize()}', {r[1]})"
        cursor.execute(query)
        conn.commit()
...
with conn.cursor() as cursor:
    ...
    for k in kingdom:
        query = f"""SELECT COUNT(att.taxon_id) FROM api_taxon_tree att
                    JOIN api_taxon at ON att.taxon_id = at.taxon_id
                    JOIN ranks r ON at.rank_id = r.id
                    WHERE r.id = 34 AND att.path LIKE '%>{k[0]}%' AND at.is_in_taiwan = 1 AND at.is_deleted != 1 
                    """
        cursor.execute(query)
        tmp = cursor.fetchone()
        # k_results += [(k[1], tmp[0])]
...
with conn.cursor() as cursor:
    # 只計算到種階層
    query = f"""SELECT alien_type, taxon_id FROM api_taxon at 
                WHERE rank_id = 34 AND at.is_in_taiwan = 1 AND at.is_deleted != 1;
                """  
    cursor.execute(query)
    source_count = cursor.fetchall()
    source_count = pd.DataFrame(source_count, columns=['alien_type', 'taxon_id'])
    tmp_alien_type = {}
    for at in source_count['alien_type']:
        at_list = []
        if at:
        ...
        for atl in at_list:
            if atl not in tmp_alien_type.keys():
                ...
                tmp_alien_type[atl] += 1
    for s in tmp_alien_type.keys():
        query = f"INSERT INTO api_web_stat (title, category, count) VALUES ('source_count', '{source_dict[s]}', {tmp_alien_type[s]})"
        cursor.execute(query)
        conn.commit()
...
with conn.cursor() as cursor:
    for e in e_group:
        ...
        query = f"""SELECT at.taxon_id FROM api_taxon at
                    JOIN taxon_names tn ON at.accepted_taxon_name_id = tn.id
                    WHERE tn.name IN {name_str} AND at.is_in_taiwan = 1 AND at.is_deleted != 1 
                """
        cursor.execute(query)
        ...
        query = f"""SELECT at.is_endemic, COUNT(att.taxon_id) FROM api_taxon_tree att
                    JOIN api_taxon at ON att.taxon_id = at.taxon_id
                    JOIN ranks r ON at.rank_id = r.id
                    WHERE r.id = 34 AND ({condition}) AND at.is_in_taiwan = 1 AND at.is_deleted != 1 
...
with conn.cursor() as cursor:
    ...
    for c in animalia_compare:
        query = f"""SELECT at.taxon_id FROM api_taxon at
                    JOIN taxon_names tn ON at.accepted_taxon_name_id = tn.id
                    WHERE tn.name = '{c[1]}' AND at.is_in_taiwan = 1 AND at.is_deleted != 1 
                """
        cursor.execute(query)
        ...
        query = f"""SELECT COUNT(att.taxon_id) FROM api_taxon_tree att
                    JOIN api_taxon at ON att.taxon_id = at.taxon_id
                    JOIN ranks r ON at.rank_id = r.id
                    WHERE r.id = 34 AND att.path LIKE '%>{c_taxon[0]}%' AND at.is_in_taiwan = 1 AND at.is_deleted != 1 
                    """
        cursor.execute(query)
        tmp = cursor.fetchone() 
...
with conn.cursor() as cursor:
    for c in kingdom_compare:
        query = f"""SELECT at.taxon_id FROM api_taxon at
                    JOIN taxon_names tn ON at.accepted_taxon_name_id = tn.id AND at.is_in_taiwan = 1 AND at.is_deleted != 1 
                    WHERE tn.name = '{c[1]}'
                """
        cursor.execute(query)
        c_taxon = cursor.fetchone()
